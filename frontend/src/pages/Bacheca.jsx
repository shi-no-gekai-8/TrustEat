...
function Bacheca() {
  ...
  const caricaMessaggi = async () => {
    try {
      // NOTA: Ho cambiato la porta a 5000 per allinearmi al backend del tuo amico
      const res = await axios.get("http://localhost:5000/api/bacheca/leggi");
  ...
  const inviaCommento = async (idMessaggio) => {
    ...
    try {
      // NOTA: Anche qui porta 5000
      await axios.put(`http://localhost:5000/api/bacheca/rispondi/${idMessaggio}`, {
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "20px" }}>
        ...
        {messaggi.map((msg) => (
          <div key={msg._id} style={{ border: "1px solid #ddd", borderRadius: "10px", padding: "20px", marginBottom: "20px", boxShadow: "0 2px 5px rgba(0,0,0,0.1)" }}>
            
            {/* IL MESSAGGIO PRINCIPALE */}
            <h3 style={{ margin: "0 0 10px 0" }}>{msg.testo}</h3>
            <small style={{ color: "gray" }}>
              Autore: {msg.autore ? msg.autore.slice(0,6) : "Anonimo"}... | {new Date(msg.dataCreazione).toLocaleString()}
            </small>
            ...
            <div style={{ backgroundColor: "#f9f9f9", padding: "10px", borderRadius: "8px" }}>
               <h4>Risposte:</h4>
               {msg.risposte.length === 0 && <p style={{fontSize: "12px"}}>Nessuna risposta ancora.</p>}
               
               {msg.risposte.map((risposta, index) => (
                  <div key={index} style={{ borderBottom: "1px solid #eee", padding: "5px 0", fontSize: "14px" }}>
                     <strong>{risposta.autoreRisposta ? risposta.autoreRisposta.slice(0,4) : "??"}..</strong>: {risposta.testoRisposta}
                  </div>
               ))}
               ...
               <div style={{ marginTop: "15px", display: "flex", gap: "10px" }}>
                  <input 
                     ...
                     onChange={(e) => setCommentiInput({ ...commentiInput, [msg._id]: e.target.value })}
                     style={{ flex: 1, padding: "8px", borderRadius: "5px", border: "1px solid #ccc" }}
                  />
                  <button 
                     onClick={() => inviaCommento(msg._id)}
                     style={{ backgroundColor: "#3498db", color: "white", border: "none", borderRadius: "5px", cursor: "pointer", padding: "0 15px" }}
                  >
                     Invia
                  </button>
               </div>
